...
export default (entity, block, { rigid, movable, collision, wallJump=false }) => {
    if (!rigid) { // default update handler
        return () => collision.test(collision.onHit)
    }
    const resolver = getResolver(entity, block, movable)
    if (resolver.oneOff) {
        return () => {
    ...
    return () => { // update handler with collison resolution logic
        ...
        movementX && collision.test(block => {
            ...
            resolver.resolveX(entity, block, movementX, movable)
            collision.onHit(block, velX, null, !(entity.pos.x === entity.prevPosX)) // last argument is a flag indicating whether the ball (moved) collided from non colliding position
            xResolved = true
        })
        ...
        movementY && collision.test(block => {
             ...
             * in this case, even after the code resolves x collision, the entity still tests positive for aabb test
            ...
            resolver.resolveY(entity, block, movementY, movable)
            collision.onHit(block, null, velY, !(entity.pos.y === entity.prevPosY))
        })
    }
}