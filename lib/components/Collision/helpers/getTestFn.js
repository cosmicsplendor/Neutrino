import { aabbCirc, aabb, fixedAabb, circCirc, circPlank } from "@lib/utils/math"
...
const testAabbCirc = (circ, rect) => {
    return aabbCirc(circBounds(circ), rectBounds(rect))
}

const testCircPlank = (circ, plank) => {
    return circPlank(circBounds(circ), rectBounds(plank))
}
...
export default (e1, e2, { rigid, movable }) => {
    ...
    if (e1Shape === shapes.CIRC && e2Shape === shapes.RECT) {
        return (e1, e2) => testAabbCirc(e1, e2)
    }
    if (e1Shape === shapes.RECT && e2Shape === shapes.CIRC) {
        return (e1, e2) => testAabbCirc(e2, e1)
    }
    if (e1Shape === shapes.CIRC && e2Shape === shapes.PLANK) {
        return (e1, e2) => testCircPlank(e1, e2)
    }
}
