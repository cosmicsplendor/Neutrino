...
class Webgl2Renderer {
    ...
    constructor({ image, cnvQry, viewport, scene, clearColor = [0, 0, 0, 0], background = "#000000" }) {
        ...
        this.tex32cache = new Float32ArrayCache()
        this.alpha32cache = new Float32ArrayCache()
        this.mat32cache = new Float32ArrayCache()
    }
    texMatCache = {}
    invCache={
        _: {},
    ...
    renderRecursively(node = this.scene) {
        ...
        if (this.instanceData.count > this.BATCH_SIZE) {
            this.flushBatch()
        }
        if (node.children) {
