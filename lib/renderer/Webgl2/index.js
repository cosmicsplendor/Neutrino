...
class Webgl2Renderer {
    _overlay = null // bool flag (gpu cache)
    _tint = null // bool flag
    _alpha = 1
    constructor({ image, cnvQry, viewport, scene, clearColor=[ 0, 0, 0, 0 ], background = "#000000" }) {
        ...
        this.uTint = gl.getUniformLocation(program, "tint") // tint flat
        this.uOverlayCol = gl.getUniformLocation(program, "overlay_col") // uniform3fv
        this.uTintCol = gl.getUniformLocation(program, "tint_col") // uniform4fv
        ...
        this.gl.uniform1f(this.uAlpha, 1)
        this.gl.uniform1i(this.uTint, 0)
        this.gl.uniform1i(this.uOverlay, 0)
    ...
    set overlay(val) {
        if (this._overlay === val) { return } // skip if overlay state hasn't changed from last time
        this._overlay = val
        this.gl.uniform1i(this.uOverlay, val ? 1: 0)
    }
    set tint(val) {
        if (this._tint === val) { return } // if tint state hasn't changed, avoid the overhead of sending data to GPU
        this._tint = val
        this.gl.uniform1i(this.uTint, val ? 1: 0)
    }
    set overlayCol(val) {
        this.gl.uniform3fv(this.uOverlayCol, val)
    }
    set tintCol(val) {
        this.gl.uniform4fv(this.uTintCol, val)
    }
    ...
    translate(x, y) {
        this.matrixUtil.translate(this.matrixStack.active, x, y)
    }
    rotate(rad) {
        this.matrixUtil.rotate(this.matrixStack.active, rad)
    }
    scale(x, y) {
        this.matrixUtil.scale(this.matrixStack.active, x, y)
    }
    save() {
    ...
    resize({ width, height }) {
        ...
        this.matrixUtil.identity(this.matrixStack.first)
        // gl_Position = vec4(clipspace * vec2(1, -1), 0, 1);
        this.matrixUtil.scale(this.matrixStack.first, 1, -1)
        // vec2 clipspace = (normalized * 2.0) - 1.0;
        this.matrixUtil.translate(this.matrixStack.first, -1, -1)
        // vec2 normalized = pos_vec / u_resolution;
        this.matrixUtil.scale(this.matrixStack.first, 2 / absWidth, 2 / absHeight)
    }
    render(node) {
        ...
        gl.uniformMatrix3fv(uMat, false, this.matrixStack.active)
