...
class Webgl2Renderer {
    api = WEBGL
    BATCH_SIZE=250 // this is a safe generic (minimum) value for a wide variety of devices including mobile
    constructor({ image, cnvQry, viewport, scene, clearColor = [0, 0, 0, 0], background = "#000000" }) {
        ...
        this.float32ArrayCache = new Float32ArrayCache()
    }
    texMatCache = {}
    invCache={
        _: {},
    ...
    uploadInstanceData() {
        ...
        gl.bufferData(gl.ARRAY_BUFFER, this.float32ArrayCache.get(this.instanceData.mat), gl.DYNAMIC_DRAW);

        gl.bindBuffer(gl.ARRAY_BUFFER, this.instanceBuffers.instanceTexMatBuffer);
        gl.bufferData(gl.ARRAY_BUFFER, this.float32ArrayCache.get(this.instanceData.texMat), gl.DYNAMIC_DRAW);

        // Upload Alpha
        gl.bindBuffer(gl.ARRAY_BUFFER, this.instanceBuffers.instanceAlphaBuffer);
        gl.bufferData(gl.ARRAY_BUFFER, this.float32ArrayCache.get(this.instanceData.alpha), gl.DYNAMIC_DRAW);
    }
    drawInstances(count) {
        if (count === 0) return
        const gl = this.gl;
