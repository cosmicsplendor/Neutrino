...
import Float32ArrayCache from "./Float32ArrayCache";
import { WEBGL } from "../apis";

class Webgl2Renderer {
    api = WEBGL
    BATCH_SIZE=75 // this is a safe generic (minimum) value for a wide variety of devices including mobile
    _useOverlay=0
    constructor({ image, cnvQry, viewport, scene, clearColor = [0, 0, 0, 0], background = "#000000" }) {
        ...
        this.tex32cache = new Float32ArrayCache(this.BATCH_SIZE * 9)
        this.mat32cache = new Float32ArrayCache(this.BATCH_SIZE * 9)
        this.alpha32cache = new Float32ArrayCache(this.BATCH_SIZE)
    }
    texMatCache = {}
    invCache={
        _: {},
    ...
    uploadInstanceData() {
        ...
        gl.bufferData(gl.ARRAY_BUFFER, this.mat32cache.get(this.instanceData.mat), gl.DYNAMIC_DRAW);

        gl.bindBuffer(gl.ARRAY_BUFFER, this.instanceBuffers.instanceTexMatBuffer);
        gl.bufferData(gl.ARRAY_BUFFER, this.tex32cache.get(this.instanceData.texMat), gl.DYNAMIC_DRAW);

        // Upload Alpha
        gl.bindBuffer(gl.ARRAY_BUFFER, this.instanceBuffers.instanceAlphaBuffer);
        gl.bufferData(gl.ARRAY_BUFFER, this.alpha32cache.get(this.instanceData.alpha), gl.DYNAMIC_DRAW);
    }
    drawInstances(count) {
        const gl = this.gl;
        gl.bindVertexArray(this.vao);
