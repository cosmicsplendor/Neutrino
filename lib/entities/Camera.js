...
class Camera extends Node {
    constructor({ subject, viewport, world, z = 1, ...nodeProps }) {
        ...
        this.onViewportChange = viewport => {
            this.bounds.width = viewport.width *  window.devicePixelRatio
            this.bounds.height = viewport.height *  window.devicePixelRatio
            this.focusInst()
        }
    ...
    focus() {
        ...
        this.pos.x += (destX - this.pos.x) / 25
        this.pos.y += (destY - this.pos.y) / 25
    }
    update() {
        this.focus()
    }
