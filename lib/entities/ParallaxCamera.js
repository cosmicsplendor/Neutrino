...
class ParallaxCamera extends Camera {
    ...
    setWorld(worldWidth, worldHeight, baseline) {
        ...
        if (worldHeight - maxFy >= this.viewport.height / 2) { // if world-camera is restrained vertically (not scrollable)
            entYOffset = (this.viewport.height / 2) * (this.z - 1) / this.z
            h += entYOffset
        } else {
            h += this.viewport.height / 2 // make sure the parallax camera too is scrollable
            entYOffset = (this.viewport.height / 2 ) * (this.z - 1) / this.z
        }
        this.world.width = w
        this.world.height = h
