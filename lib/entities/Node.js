class Node {
    ...
    static updateRecursively(node, dt, t, rootNode = node) {
        ...
        node._visible = rootNode.intersects ? rootNode.intersects(node): true
        node._visible && node.update && node.update(dt, t)
        if (!node.children) { return }
         ...
         * Here's an interestin fact: this shrinking of children array results in the removed node's next sibling shifting an index backward,
    ...
    static removeChild(parentNode, childNode) {
        parentNode.children = parentNode.children.filter(n => n !== childNode)
        this.cleanUpRecursively(childNode)
    }
    ...
    remove() {
        Node.removeChild(this.parent, this)
    }
}
