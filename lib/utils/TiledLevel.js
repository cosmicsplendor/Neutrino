...
const _resettable = factory => (x, y, props) => { // returns a new factory same as the input, except that it's instances have reset method 
    ...
    ent.reset = !!ent.reset ? ent.reset : () => Object.assign(factory, { pos: { x, y }, ...props})
    return ent
}
...
class TiledLevel extends  Node {
    ...
    constructor({ player, data, texatlas, factories={}, bg, fbg, ...nodeProps }) {
        // if there isn't a player factory in the factories dict, create a new one
        factories.player = factories.player ? factories.palyer: (x, y) => { // a pseudo player factory that reuses the player passed into the map, and gives it to "player" spawn-point handler
            player.pos.x = x
            player.pos.y = y
