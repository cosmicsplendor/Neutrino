...
class TiledLevel extends  Node {
    ...
    constructor({ player, data, factories={}, ...nodeProps }) {
        ...
        const addSpawnPoint = ({ x, y, groupId, layer = "w", ...props }) => {
            ...
            if (name === "player" && props.temp && !config.testMode) return
            if (name === "player" && config.testMode && !props.temp) return
            if (name === "player" && playerAdded) return
        ...
        checkpoints.forEach(p => {
            fgObjNode.add(new Rect({ width: 64, height: 64, fill: "white", pos: { x: p.x, y: p.y }, alpha: 0.5 }))
        })
        ...
        data.collisionRects.forEach(({ x, y, width, height } )=> {
            fgObjNode.add(new Rect({ width, height, pos: { x, y }, alpha: 0.25, fill: "#aaa", stroke: "black", strokeWidth: 5 }))
        })
    }
}
