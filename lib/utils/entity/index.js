...
export const  getReusableBounds = (() => {
    const bounds = Array(2).fill(0).map(() => ({ x: 0, y: 0, width: 0, height: 0, normalX: 0, normalY: 0 }))
    let lastAccessedIdx = 0
    return (x, y, width, height, normalX, normalY) => {
        ...
        bounds[lastAccessedIdx].normalX = normalX
        bounds[lastAccessedIdx].normalY = normalY
...
export const getReusablePlank =(() => {
    const plank = { x: 0, y: 0, width: 0, height: 0, normalX: 0, normalY: 0 }
    return (x, y, width, height, normalX, normalY) => {
        plank.x = x
        plank.y = y
        plank.width = width
        plank.height = height
        plank.normalX = normalX
        plank.normalY = normalY
        return plank
    }
})()
...
export function rectBounds(ent) {
    ...
    if (ent.hitbox) {
        return getReusableBounds(globalPos.x + ent.hitbox.x, globalPos.y + ent.hitbox.y, ent.hitbox.width, ent.hitbox.height, ent.normalX, ent.normalY)
    }
    return getReusableBounds(globalPos.x, globalPos.y, ent.width, ent.height, ent.normalX, ent.normalY)
}
