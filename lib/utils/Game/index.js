...
class Game {
    _activeScreen = null
    _sounds = []
    constructor({ renderer, assetsCache, screenFactories, activeScreen, ...rest }) {
        ...
        SoundSprite.onDestroyed = soundObj => { this._sounds = this._sounds.filter(s => s !== soundObj ) }
    ...
    pause() {
        ...
        const playingSounds = this._sounds.filter(sound => sound.playing) // sounds that were playing before pause call
